var Visitor, python_module, op, PythonVisitor, visitor;
Visitor = require('./base').Visitor;
python_module = require('./validatr.py').source;
op = {
  "is": " == ",
  "is not": " != ",
  ">": " > ",
...
PythonVisitor = (function(_super){
  PythonVisitor.displayName = 'PythonVisitor';
  var prototype = __extends(PythonVisitor, _super).prototype, constructor = PythonVisitor;
  function PythonVisitor(){
    PythonVisitor.superclass.apply(this, arguments);
    this.indentation = 2;
    this.declarations = [];
  }
  ...
  prototype.visit = (function(){
    function visit(node, ind){
      ...
      s = PythonVisitor.superclass.prototype.visit.call(this, node);
      s = this.indent(s);
  ...
  prototype.opts = (function(){
    function opts(node, res){
      if (node.validation) {
        res.push("validation=lambda x, ctx: " + this.visit(node.validation, 0));
      }
      if (node.assign) {
...
visitor = new PythonVisitor({
  ...
  TypeRef: (function(){
    function TypeRef(node){
      var decl;
      decl = node.name + " = " + this.visit(node.type, 0);
  ...
  CIf: (function(){
    function CIf(node){
      var args;
      args = [];
      args.push("lambda x, ctx: " + this.visit(node.cond));
